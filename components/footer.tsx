...
export function Footer({ theme = "light" }: FooterProps) {
  ...
  return (
    <footer
      id="site-footer"
      className="relative overflow-hidden border-t border-dhakaa-200/70 bg-white"
    >
      <div className="relative z-10 mx-auto w-full max-w-6xl px-6 pt-5 pb-20 sm:px-8 sm:py-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div className="flex flex-col items-start gap-2">
            ...
            />
            <p className="font-subtitle max-w-xs text-left text-xs leading-snug tracking-[0.005em] text-dhakaa-600">
              Custom premium software for established companies.
            </p>
          </div>
            ...
            >
              {navLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="text-xs text-dhakaa-600 transition-colors duration-150 ease-out hover:text-blue-600"
                >
                  {link.label}
                </Link>
              ))}
            </nav>
            <p className="text-right text-xs text-dhakaa-500">
              © {new Date().getFullYear()} All rights reserved.
            </p>
          </div>
        </div>
